...
A small vocabulary trainer for the school year. The interface is in French —
the language the learner already speaks — while the vocabulary being practised
is German (and later English). Two ways to practise each unit:
...
Published with GitHub Pages from the repository root:
    ...
    https://flc-lab-projects.github.io/Revision/allemand/

## Changing the interface wording

Every word the app displays lives in one object called `T`, near the top of the
`<script>` block in `index.html`. Change it there and nowhere else. The app title
and tagline come from `title` and `tagline` in `content/index.json`.
...
Same thing with `"language": "Anglais"`, `"answer": "anglais"` and files under `content/en/`. As soon as
